...
А что бы понять в чем суть проблемы, нужно сначала понять принцип действия техники `void_t`  
...
А валидным он может быть только и только в том случае, если внутри `T` окажется доступным тип `value_type`  
...
В этом случае, согласно правилам SFINAE, компилятор должен выбрать оставшиёся единственным главный шаблон:  
```
template<class, class = void> struct has_value_type
    : std::false_type
{};
...
Это произошло потому, что компилятор и не пытался распарсить `typename T::value_type`  
...
На это в стандарте есть железобетонная отмазка: делай нормально, и будет у тебя нормально.  
...
C помощью дискриминатора можно заставить компилятор честно проверить все параметры шаблона:  
