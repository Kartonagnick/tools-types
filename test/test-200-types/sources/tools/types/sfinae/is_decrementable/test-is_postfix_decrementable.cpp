...
// [2021y-04m-21d][13:12:55] Idrisov Denis R.
...
#define dTEST_METHOD is_postfix_decrementable
...
namespace
{
    ...
    #define dEXPRESSION(a,v) \
        me::is_postfix_decrementable<a>::value == v

    #ifdef dHAS_STATIC_ASSERT
        #define dCHECK(type, expected)                                  \
            static_assert(                                              \
                dEXPRESSION(type, expected),                            \
                "'tools::is_postfix_decrementable<" # type ">::value' " \
                "must be '" #expected "'"                               \
            )
    #else
        #define dCHECK(type, expected)      \
