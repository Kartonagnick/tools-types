...
// [2021y-04m-21d][13:13:44] Idrisov Denis R.
...
#define dTEST_METHOD is_postfix_incrementable
...
namespace
{
    ...
    #define dEXPRESSION(a,v) \
        me::is_postfix_incrementable<a>::value == v

    #ifdef dHAS_STATIC_ASSERT
        #define dCHECK(type, expected)                                  \
            static_assert(                                              \
                dEXPRESSION(type, expected),                            \
                "'tools::is_postfix_incrementable<" # type ">::value' " \
                "must be '" #expected "'"                               \
            )
    #else
        #define dCHECK(type, expected)      \
